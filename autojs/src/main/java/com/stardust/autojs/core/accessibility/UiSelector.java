...
//        Log.d("autoxjsv6","UiSelector : public UiObjectCollection find()");
        return find(Integer.MAX_VALUE,false);
    }
    // <
...
//            Log.d(TAG, "find: roots = " + roots);
        if (roots.isEmpty()) {
//            Log.d("autoxjsv6","UiSelector : protected UiObjectCollection findImpl(int max) : roots.isEmpty()");
        ...
        for (AccessibilityNodeInfo root : roots) {
            ...
            result.addAll(findAndReturnList(UiObject.Companion.createRoot(root, mAllocator), max - result.size(),isRefresh));
            if (result.size() >= max) {
                break;
            }
        }
