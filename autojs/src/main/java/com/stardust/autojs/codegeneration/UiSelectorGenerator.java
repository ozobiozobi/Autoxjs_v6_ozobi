...
public class UiSelectorGenerator {
    ...
    private boolean shouldStopGeneration(UiGlobalSelector selector) {
        if (mSearchMode == UNTIL_FIND) {
            return !selector.findAndReturnList(mRoot, 1,false).isEmpty();// Modified by ibozo - 2024/10/31
        } else {
            return selector.findAndReturnList(mRoot, 2,false).size() == 1;// Modified by ibozo - 2024/10/31
